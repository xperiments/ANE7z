    ...
    if (res == SZ_OK)
    {
        ...
        if (extractCommand)
        {
            ...
            for (i = 0; i < db.db.NumFiles; i++)
            {
                ...
                // ANE7Z ADDED DISPATCH PROGRESS EVENT
                char buffer [255];
                sprintf(buffer, "{\"current\":%d,\"total\":%d}",(i+1), db.db.NumFiles );
                DISPATCH_STATUS_EVENT( context, (uint8_t*)"UN7Z_PROGRESS", (uint8_t*)buffer );
            }
            SzArEx_DictCache_free(&dictCache);
        }
    }
